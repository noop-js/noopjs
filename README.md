...
**JavaScript bundles must shrink.** The average React page ships ~45 KB of framework JS. NoopJS ships 0 KB for static pages. Interactive pages ship only the exact handler code, lazily loaded on first click. The resumer is 2.75 KB gzipped.
...
| **Client resumer** | ~2.75 KB gzipped. Re-attaches signals to DOM without re-running components. |
